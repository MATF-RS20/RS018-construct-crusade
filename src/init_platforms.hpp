...
void init_platforms_and_enemies(std::vector<BigPlatform> &big_platforms,
    ...
    for(int j = -3; j < 6; j++){
        big_platforms.push_back(BigPlatform(platform_distance*j, fixed_platform_height, 8 + rand() % 3, platform_sprite, 0));
    ...
    for(int j = 1; j < 6; j++){
        big_platforms.push_back(BigPlatform(6.3*platform_distance + platform_distance/2.5*(pow((-1),j)),
                                            fixed_platform_height - j*platform_height_offset , 7 + rand() % 3, platform_sprite, 0));
        player.num_of_platforms_++;
        if(rand() % 101 < 40)
    ...
    for(int j = 5; j > 0; j--){
        big_platforms.push_back(BigPlatform(platform_distance*j, level_1_height, 8 + rand() % 3, platform_sprite, 0));
        player.num_of_platforms_++;
    }
    ...
    for(int j = 0 ; j > -6; j--){
        big_platforms.push_back(BigPlatform(platform_distance*j,level_1_height - j*platform_height_offset,
                                            8 + rand() % 3, platform_sprite, 0));
        player.num_of_platforms_++;
    }

    //level 0 again - height 500 - this is gonna be a special area at some point
    big_platforms.push_back(BigPlatform(platform_distance*(-7), fixed_platform_height, 25, platform_sprite, 0));
    player.num_of_platforms_++;
    ...
    big_platforms.push_back(BigPlatform(platform_distance, level_2_height, 25, platform_sprite, 0));
    player.num_of_platforms_++;
